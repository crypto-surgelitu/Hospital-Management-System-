...
if (!empty($national_id)) {
    if (strlen($national_id) < 6 || strlen($national_id) > 8) {
        ...
        $check = $pdo->prepare("SELECT COUNT(*) FROM patients WHERE national_id = ? AND patient_id != ?");
        $check->execute([$national_id, $patient_id]);
...
if (!empty($errors)) {
    ...
    $_SESSION['old'] = $_POST;
    header('Location: /modules/patients/edit.php?id=' . $patient_id);
    exit;
}

$stmt = $pdo->prepare("
    UPDATE patients 
    SET full_name = ?, gender = ?, date_of_birth = ?, phone = ?, email = ?, address = ?, national_id = ?
    WHERE patient_id = ?
");
$stmt->execute([
    $full_name,
...
header('Location: /modules/patients/view.php?id=' . $patient_id . '&success=updated');
exit;